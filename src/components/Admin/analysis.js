...
export default function AdminAnalysis() {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "http://localhost:5000/api/admin/question-by-month"
        );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(
          "http://localhost:5000/api/admin/question-by-year"
        );
        const data = await response.json();
        data.forEach((item) => {
      ...
      >
        ...
        <div className="charts">
          <div className="first_row">
            <Chart
              title="overall data"
              count={[user, NoOfQuestions, answer, accept]}
              Tags={["Users", "Questions", "Answers", "Accepted Answers"]}
            />
            <Chart
              title="No of questions in particular tag"
              count={count}
              Tags={Tags}
            />
          </div>
          ...
          <div className="first_row">
            <Chart
              title="No of questions in particular month"
              count={[
                january,
                february,
                march,
            ...
            />
            <Chart
              title="No of questions in particular year"
              count={[twentyone, twentytwo, twentythree]}
              Tags={["2021", "2022", "2023"]}
            />
          </div>
        </div>
