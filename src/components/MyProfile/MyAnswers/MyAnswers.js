...
export default function Profile() {
  ...
  const fetchAllFilteredAnswers = async () => {
    const response = await fetch(
      `http://localhost:5000/api/answer/fetchUserFilteredAnswers/${localStorage.getItem(
        "username"
      )}`,
      {
        method: "POST",
  ...
  useEffect(() => {
    fetch(
      `http://localhost:5000/api/answer/givenAnswersTags/${localStorage.getItem(
        "username"
      )}`,
      {
        method: "POST",
  ...
  useEffect(() => {
    fetch(
      `http://localhost:5000/api/answer/fetchUserAnswers/${localStorage.getItem(
        "username"
      )}`,
      {
        method: "POST",
